...
$cert ='.\Program Files (x86)\Windows Kits\8.0\App Certification Kit\appcert'
$guid = [guid]::NewGuid().ToString() + ".xml"
$reportPath = $resultPath + $guid

& $cert  reset
& $cert test -apptype metrostyle -AppxPackagePath $appxfile.fullname -reportoutputpath $reportPath
